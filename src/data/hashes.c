...
static unsigned int hash_int_pair(int c1, int c2) {
  // Calcula o maior e o menor códgigo para ordena-los de uma forma exeta ao
  // invés da ordem dos argumentos
  int minn = c1 < c2 ? c1 : c2;
  int maxx = c1 > c2 ? c1 : c2;

  // Combinação simples dos dois inteiros para gerar um hash, usando números
  // primos para reduzir colisões
...
int inserir_estacao(HashEstacao *ht, const char *nome) {
  // Verifica se a hash table ou o nome são nulos, e retorna 0 para indicar
  // falha
  if (!ht || !nome)
    return 0;

  // Calcula o hash do nome da estação e retorna o índice da bucket
  // correspondente
  ...
  while (curr) {
    // Se a estação já existe, retorna 0 para indicar que a inserção não foi
    // realizada
  ...
  // Se chegou aqui, a estação é nova e deve ser inserida no início da lista
  // encadeada da bucket
...
int inserir_par(HashPar *ht, int c1, int c2) {
  ...
  // Calcula o hash do par de estações e retorna o índice da bucket
  // correspondente
  unsigned int key = hash_int_pair(c1, c2);
  NDPAR *curr = ht->buckets[key];

  // Percorre a lista encadeada da bucket para verificar se o par de estações já
  // existe
  while (curr) {
