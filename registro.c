...
void read_from_bin(FILE *p_bin, REG *reg)
{
    ...
    if (fread(&reg->codProxEstacao, sizeof(reg->codProxEstacao), 1, p_bin) != 1)
        return;
    if (fread(&reg->distProxEstacao, sizeof(reg->distProxEstacao), 1, p_bin) !=
        1)
        return;
    if (fread(&reg->codLinhaIntegra, sizeof(reg->codLinhaIntegra), 1, p_bin) !=
        1)
        return;
...
void write_in_bin(FILE *f_bin, REG *reg)
{
    ...
    // Escreve os bytes de lixo no final do registro para completar os 80 bytes
    // do registro
    char lixo = '$';
